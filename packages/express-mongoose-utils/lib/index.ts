...
export class DbController<T> implements Controller {
  ...
  getDetail =
    ...
    async (req: Request, res: Response) => {
      ...
      return res.send(
        successResponse({ data: { [self._metaData[1]]: detail } })
      );
    };
  ...
  getList =
    ...
    async (req: Request, res: Response) => {
      ...
      const filterArray = Object.keys(self._filter)
        ...
        .map((key) => ({
          [key]:
            typeof self._filter[key] === 'string'
              ? req.query[key]
              : self._filter[key](req.query[key]),
        }));
      // TODO: Remove any and add key from T
      const filter: any = filterArray.length
        ? filterArray.reduce((p, n) => ({ ...p, ...n }))
        : {};
  ...
  checkExists =
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const isExist = await self._service.exists({
          _id: req.params[self._metaData[0]],
        });
        if (isExist) {
          return next();
